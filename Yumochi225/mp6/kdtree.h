...
class KDTree
{
  public:
    ...
    /**e to the terminal in a pretty way.
     */
  ...
  private:
     ...
     */
     void sort( int start, int end, int dimension);
     void select(int start, int end, int n, int dimension);
     int partition(int start, int end, int pivotIndex, int dimension);
     void swap(Point<Dim>& first, Point<Dim>& second);
     int calDist(const Point<Dim>& first, const Point<Dim>& second) const;
     Point<Dim> findNearestNeighbor(int start, int end, const Point<Dim>& query,
     int dimension) const;
