...
void testShouldReplace()
{
    ...
    cout << "shouldReplace(target, currentBest1, possibleBest1) = "
         << tree.shouldReplace(target, currentBest1, possibleBest1)
         << endl; // should print true (1)
    cout << "shouldReplace(target, currentBest2, possibleBest2) = "
         << tree.shouldReplace(target, currentBest2, possibleBest2)
         << endl; // should print false (0)
    cout << "shouldReplace(target, currentBest3, possibleBest3) = "
         << tree.shouldReplace(target, currentBest3, possibleBest3)
...
void testDeceptiveNNOneLevel()
{
    ...
    cout << "findNearestNeighbor(" << target
         << ") result   = " << tree.findNearestNeighbor(target) << endl;
    cout << "findNearestNeighbor(" << target << ") expected = " << expected
         << endl;
...
void testDeceptiveMines()
{
    ...
    double coords[20][2] = {{84, 44},  // mine
                            ...
                            {82, 20}}; // mine
    bool isMine[20]
        = {1, 1, 1, 1, 1, 0, 1, 0, 0, 1, 1, 1, 1, 0, 1, 1, 0, 0, 1, 1};
...
void testTieBreaking()
{
    ...
    cout << "findNearestNeighbor(" << target2
         << ") result   = " << tree.findNearestNeighbor(target2) << endl;
    cout << "findNearestNeighbor(" << target2 << ") expected = " << expected2
         << endl;
