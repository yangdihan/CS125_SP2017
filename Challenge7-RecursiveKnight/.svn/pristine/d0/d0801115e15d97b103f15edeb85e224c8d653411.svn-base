...
//		 TODO: Implement moveOne
	...
	static int findShortestString(String[] paths, int lo, int hi) {
		if (lo == hi)
			return hi;
		int thisL = safeStringLength(paths[lo]);
		int restIndex = findShortestString(paths, lo+1, hi);
		int restL = safeStringLength(paths[restIndex]);
		if (thisL <= restL)
			return lo;
		else
			return restIndex;
	...
	public static String shortestPath(int x, int y, int tX, int tY, boolean blocked[][]) {
		...
		paths[3] = shortestPath(x-1, y, tX, tY, blocked);//W
		...
		if ((paths[0] == null)&&(paths[1] == null)&&(paths[2] == null)&&(paths[3] == null))
			return null;
		int shortest = findShortestString(paths, 0, 3);
		String ans = "";
		if (shortest == 0)
			ans = "N";
		if (shortest == 1)
		...
		if (shortest == 3)
			ans = "W";
		ans += paths[shortest];
		//System.out.println(paths[shortest]); 
	...
	public void chase(boolean maze[][], int targetX, int targetY) {
		...
		int x = this.x;
		int y = this.y;
		String shortestPath = shortestPath(x, y, targetX, targetY, maze);
		if (shortestPath != null){
			if (shortestPath != ""){
				char heading = shortestPath.charAt(0);
				this.moveOne(heading);
			}
		}
